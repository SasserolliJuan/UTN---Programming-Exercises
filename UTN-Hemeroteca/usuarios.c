...
nodoListaUsuario *agregarPublicacionAHistorial(lista_de_listas **listaPublicaciones, nodoListaUsuario *listaUsuarios, char emailUsuario[], datosCadaPublicacion nuevaPublicacion, fila *filaPrestadas)
{
    ...
    {
        // Crear un nuevo nodo de historial
        nodoHistorial *nuevoHistorial = crearNodoHistorial(nuevaPublicacion);

        // Verificar si la publicacion esta prestada
        if (nuevoHistorial->dato.prestado == 0)
        {
            nuevoHistorial->dato.prestado = 1;

            // Obtener la fecha actual como tiempo de reserva
            time_t tiempoReserva = time(NULL);

            // Agregar la publicacion a la fila
            agregarAFila(filaPrestadas, nuevoHistorial->dato, tiempoReserva);
        }

        // Incrementar la cantidad de veces leidas y actualizar el historial del usuario
        ...
        // Agregar el nuevo historial al inicio de la lista de historial del usuario
        nuevoHistorial->sig = usuarioEncontrado->listaHistorial;
        usuarioEncontrado->listaHistorial = nuevoHistorial;
