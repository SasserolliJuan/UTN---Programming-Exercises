...
lista_de_listas *archivo2ListaDeListas(char archivoRegistros[], lista_de_listas *lista)
{
...
void generarInformesYEstadisticas(lista_de_listas *lista)
{
    // Variables para almacenar las estaditicas
    ...
    {
        // Recorrer la sublista de cada genero
        ...
        while (segPublicacion != NULL)
        {
            ...
            // Verifica si la publicacion actual tiene maa lecturas que el maximo registrado.
            if (segPublicacion->miRegistro.cantVecesLeida > maxVecesLeidas)
            {
                maxVecesLeidas = segPublicacion->miRegistro.cantVecesLeida;

                // Almacena el nombre del autor de la publicacion actual como el autor mas destacado.
    ...
    printf("Total de veces leidas: %d\n", totalVecesLeidas);
    printf("Autor mas destacado: %s\n", autorMasDestacado);
    printf("------------------------------------\n");
}
